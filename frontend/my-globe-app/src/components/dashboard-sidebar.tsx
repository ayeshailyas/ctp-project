...
export default function DashboardSidebar({ countryCode, countryName, onClose }: DashboardSidebarProps) {
  ...
  // New State: Controls if the papers panel is actually visible
  ...
  // Get Papers for the active subfield
    ...
    <>
      <div 
        className={`fixed top-0 z-40 h-screen w-96 border-r border-l border-slate-800 bg-black/95 backdrop-blur-xl transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] ${
           isOpen && showPapers 
             ? "translate-x-0 right-[42rem] opacity-100 shadow-2xl" // Visible position
             : "translate-x-20 right-[42rem] opacity-0 pointer-events-none" // Hidden (slid under main sidebar)
        }`}
      >
        <div className="flex h-full flex-col p-6">
          ...
          <div className="mb-6 flex items-center justify-between border-b border-slate-800 pb-4">
             <div className="flex items-center gap-2 text-indigo-400">
                ...
                <h3 className="font-bold text-white">Top Papers</h3>
             </div>
             <button 
          ...
          <div className="mb-4">
             ...
             <h4 className="text-xl font-bold text-indigo-200">{activeSubfield}</h4>
          </div>

          {/* Paper List */}
          <div className="flex-1 overflow-y-auto pr-2 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-700">
            <div className="space-y-4">
                {activePapers && activePapers.length > 0 ? (
                  activePapers.map((paper, i) => (
                    <div 
                      key={i} 
                      className="group relative flex flex-col gap-3 rounded-xl border border-slate-800 bg-slate-900/50 p-4 transition-all hover:border-indigo-500/50 hover:bg-indigo-950/20"
                    >
                       <div className="flex items-start justify-between gap-3">
                          <h5 className="text-sm font-medium text-slate-200 leading-snug group-hover:text-indigo-300 transition-colors">
                            {paper.title}
                          </h5>
                       </div>
                       ...
                       <div className="flex items-center justify-between pt-2">
                          <div className="flex items-center gap-3 text-xs text-slate-500">
                             <span className="flex items-center gap-1 rounded bg-slate-800 px-1.5 py-0.5">
                               {paper.year}
                             </span>
                  ...
                  <div className="flex flex-col items-center justify-center py-10 text-center text-slate-600">
                    <p className="text-sm">No specific paper data available.</p>
                  </div>
                )}
            </div>
          </div>
          
          <div className="mt-4 border-t border-slate-800 pt-4 text-center text-[10px] text-slate-600">
            Top 3 most cited papers in the last 20 years.
          </div>
        </div>
      </div>
                ...
                <div className="h-[400px] w-full cursor-pointer">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      ...
                      onClick={handleBarClick} // Trigger the sidebar extension
                    >
